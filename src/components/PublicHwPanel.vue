<template>
    <el-row :gutter="20" style="width: 100%;">
        ...
        <el-col :xs="24" :sm="8" :md="8">
            <el-button round style="width: 100%" class="a-card" :class="hw.status == 0 ? (hw.subStatus == 0 ? '' : (hw.subStatus == 1 ? 'unsubmitted' : 'expired')) :
                'submitted'" @click="addhwdialog=true">
                {{ hw.status == 0 ? (hw.subStatus == 0 ? '⏰待提交' : (hw.subStatus == 1 ? '⚠️待补' : '⚠️已过期')) : '✅已完成' }}
            </el-button>
        </el-col>
    </el-row>
    
    <el-dialog align-center v-model="addhwdialog" width="90%" :title="hw.title" v-if="hw"  destroy-on-close append-to-body >
